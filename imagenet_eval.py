...
def main(args):
    ...
    # download the model from "https://openaipublic.azureedge.net/clip/models/5806e77cd80f8b59890b7e101eabd078d9fb84e6937f9e85e4ecb61988df416f/ViT-B-16.pt"
    clip_model = torch.jit.load(os.path.join('./trained_models', "{}.pt".format('ViT-B-16')))#.to(device).eval()

    print("Encoding training images...")
    val_loader = data_utils.tinyimage_loader(train = True)
    ...
    test_loader = data_utils.tinyimage_loader(train = False)
    ...
    #dictionary = list(set(dictionary)-set(test_loader.dataset.targets))
    ...
    for normal_target_set in val_loader.dataset.semantic_splits:
        all_class_scores = []
        ...
        for single_class in normal_target_set:
            ...
            normal_val_image_features = val_image_features[np.where(np.array(val_loader.dataset.targets) == single_class)]

            single_class_score = np.zeros(len(test_image_features))

            # similarity to normal text labels from datasets)
            i_scores = internal_class_score(normal_val_image_features, test_image_features, dataset_text_features[np.where(np.array(val_loader.dataset.classes) == single_class)])
